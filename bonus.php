...
<?if($cf_bonus < $site['crf_bonus']){?>
	<div><div class="err1"><?=lang_rep($lang['b_225'], array('-NUM-' => $site['crf_bonus'], '-REM-' => ($site['crf_bonus'] - $cf_bonus)))?></div></div>
<?}else{?>
...
<?}}else{?>
	<div ><div class="err1"><?=lang_rep($lang['b_41'], array('-TIME-' => r_time(($data['daily_bonus']+86400)-time())))?></div></div>
<?}?>
</div>	
<?include('footer.php');?>