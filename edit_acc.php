...
if(isset($_POST['change_pass'])){
	if (!checkPwd($_POST['password'],$_POST['password2'])) {
		$mesaj = '<div  ><div class="err1">'.$lang['b_63'].'</div></div><br>';
	}else{
		$enpass = MD5($_POST['password']);
		$db->Query("UPDATE `users` SET `pass`='".$enpass."' WHERE `id`='".$data['id']."'");
		$mesaj = '<div  ><div class="suc1">'.$lang['b_64'].'</div></div><br>';
	}
...
if(isset($_POST['change_info'])){
	...
	if($gender != 1 && $gender != 2){
		$mesaj = '<div  ><div class="err1">'.$lang['b_208'].'</div></div><br>';
	}elseif(!in_array($country, $ctrs) || $country == '0'){
		$mesaj = '<div  ><div class="err1">'.$lang['b_209'].'</div></div><br>';
	}elseif($data['c_changes'] >= $change_limit){
		$mesaj = '<div  ><div class="err1">'.lang_rep($lang['b_222'], array('-NUM-' => $change_limit)).'</div></div><br>';
	}else{
		$db->Query("UPDATE `users` SET `country`='".$country."', `c_changes`=`c_changes`+'1', `sex`='".$gender."' WHERE `id`='".$data['id']."'");
		$mesaj = '<div  ><div class="suc1">'.$lang['b_211'].'</div></div><br>';
	}
}
?>
<div class="content">
	...
	<div class="infobox t-left">
		<form method="post">
			...
			<p>
				...
				<br><div class="war1"><?=lang_rep($lang['b_207'], array('-NUM-' => ($change_limit-$data['c_changes']) < 0 ? '0' : ($change_limit-$data['c_changes'])))?></div>
			</p>
...
/*if(isset($_POST['del_acc'])){
	...
	if($db->QueryGetNumRows("SELECT id FROM `users` WHERE `id`='".$data['id']."' AND `pass`='".$pass."'") == 0){
		$mesaj = '<div  ><div class="err1"><b>err1:</b> Wrong password!</div></div><br>';
	}else{
