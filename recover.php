...
if(isset($_POST['send'])) {
	...
	if(!$captcha_valid){
		$mesaj = '<div class="err1">'.$lang['b_54'].'</div>';
	}elseif($_POST['email'] == ""){
		$mesaj = '<div class="err1">'.$lang['b_65'].'</div>';
	}elseif($rec['login'] == ""){
		$mesaj = '<div class="err1">'.$lang['b_110'].'</div>';
	}else{
...
Best Regards!";
		...
		$mesaj = '<div class="suc1">'.$lang['b_111'].'</div>';
	}
}?>
<div class="content t-left"><h2 class="title"><?=$lang['b_112']?></h2><div><?echo $mesaj;?></div>
	<div class="infobox">
		<form id="form" method="post">
			<p>
