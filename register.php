...
if(isset($_GET['resend'])){
	if(isset($_POST['resend'])){
		...
		if(!$captcha_valid){
			...
			$mesaj = '<div  ><div class="success">'.$lang['b_190'].'</div></div>';
		}
	}
?>
...
<div class="content-ex" style="text-align:left"><?=$mesaj?>
	<form action="" method="post">
			<p>
				...
				<input class="text-max1" type="email" value="<?=(isset($_POST['email']) ? $_POST['email'] : '')?>" name="email" required="required" />
			</p>
			<p>
				<?
				if($site['captcha_sys'] == 1){
...
if($site['reg_status'] == 0){
	...
	if(isset($_POST['register'])){
		...
		if(!$captcha_valid){
			...
			$mesaj = '<div  ><div class="err1">'.$lang['b_127'].'</div></div>';
		}elseif($site['more_per_ip'] != 1 && $db->QueryGetNumRows("SELECT id FROM `users` WHERE `IP`='".$IP."' OR `log_ip`='".$IP."'") > 0) {
			$mesaj = '<div  ><div class="err1"><b>'.$lang['b_128'].'</b></div></div>';
		}elseif($email != $email2) {
			$mesaj = '<div  ><div class="err1">'.$lang['b_279'] .'</div></div>';
		}elseif(!isUserID($name)) {
			$mesaj = '<div  ><div class="err1">'.$lang['b_129'].'</div></div>';
		}elseif(!isEmail($email)) {
			$mesaj = '<div  ><div class="err1">'.$lang['b_65'] .'</div></div>';
		}elseif($gender != 1 && $gender != 2) {
			$mesaj = '<div  ><div class="err1">'.$lang['b_208'].'</div></div>';
		}elseif($c_done == 0 && !in_array($country, $ctrs)) {
			$mesaj = '<div  ><div class="err1">'.$lang['b_209'].'</div></div>';
		}elseif(!checkPwd($pass1,$pass2)) {
			$mesaj = '<div  ><div class="err1">'.$lang['b_63'].'</div></div>';
		}else{
			...
			$mesaj = '<div  ><div class="success">'.$lang['b_131'].' '.($site['reg_reqmail'] == 0 ? $lang['b_132'] : $lang['b_133']).'</div></div>';
		}
	}
?>
<style>
input.text-max1 {
background: url(images/formbg.png) repeat-x #FFF;
min-width: 95%;
...
<div class="content-ex" style="text-align:left"><?=$mesaj?>
	<form action="" method="post">
			<p class="reg_row_1">
				...
				<input class="regs" type="text" value="<?=(isset($_POST['user']) ? $_POST['user'] : '')?>" name="user" required="required" />
			</p>
			<p class="reg_row_2">
			<p class="reg_row_1">
				<label><?=$lang['b_70']?></label><br />
				<input class="regs" type="email" value="<?=(isset($_POST['email']) ? $_POST['email'] : '')?>" name="email" required="required" />
			</p>
			<p class="reg_row_2">
				<label><?=$lang['b_278']?></label><br />
				<input class="regs" type="email" name="email2" required="required" />
			</p>
			<p class="reg_row_1">
				<label><?=$lang['b_15']?></label><br />
				<input class="regs" type="password" value="" name="password" required="required" />
			</p>
			<p class="reg_row_2">
				<label><?=$lang['b_72']?></label><br />
				<input class="regs" type="password" value="" name="password2" required="required" />
			</p>
			<p class="reg_row_1">
	...
	<span style="float:right"><b><a href="register.php?resend" style="text-decoration:none"><font color=white ><?=$lang['b_227']?></font></a></b></span>
</div>
<?}else{?>
	<div class="content-ex"><div><div class="err1"><?=$lang['b_134']?></div></div></div>
<?}}
include('footer.php');?>