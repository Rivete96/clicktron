...
if(isset($_POST['submit']) && $timecheck < time() && $valid_transfer) {
	...
	if($data['coins'] < $amount){
		...
		$msg = '<div><div class="suc1">'.lang_rep($lang['b_149'], array('-SENT-' => $amount, '-USER-' => $username, '-RECEIVED-' => round($tamount), '-FEE-' => $site['transfer_fee'])).'</div></div> <script> $("#c_coins").html('.($data['coins']-$amount).'); </script>';
	}
}?>
<div class="content t-left"><?=$msg?> 
<h2 class="title"><?=$lang['b_11']?></h2>
<?if($site['transfer_status'] == 2 && $data['premium'] == 0){?>
<div><div class="not1"><?=$lang['b_231']?></div></div>
