...
if(isset($_POST['vip_coins'])){
	...
	if($db->GetNumRows($sql) > 0){
		...
		if($data['coins'] >= $vc_pack['coins_price']){
			...
			$mesaj = '<div ><div class="suc1">'.$lang['b_241'].'</div></div>';
		}else{
			$mesaj = '<div ><div class="err1">'.$lang['b_146'].'</div></div>';
		}
	}
}
if(isset($_GET['history'])){
?>
<div class="content"><h2 class="title"><?=$lang['b_247']?> <span style="float:right"><a class="bbut" href="vip.php"><?=$lang['b_08']?></a></span></h2>
	<table class="table">
		<thead>
			<tr>
				<td><?=$lang['b_248']?></td>
...
<?
	...
	if(isset($_POST['submit']) && isset($_POST['pid'])){
		...
		if($db->GetNumRows($sql) == 0){
			$mesaj = '<div><div class="err1">'.$lang['b_262'].'</div></div>';
		}elseif($data['account_balance'] < $pack['price']){
			$mesaj = '<div><div class="err1">'.$lang['b_263'].' <a href="bank.php"><u>'.$lang['b_256'].'...</u></a></div></div>';
		}else{
			...
			$mesaj = '<div><div class="suc1">'.lang_rep($lang['b_264'], array('-NUM-' => $pack['coins'].' '.$lang['b_246'])).'</div></div>';
		}
	}
?>
...
<div class="content"><h2 class="title"><?=$lang['b_08']?> <span style="float:right"><a class="bbut" href="vip.php?history"><?=$lang['b_247']?></a></span></h2><?=$mesaj?> 
<div class="infobox" style="text-align:left;width:700px;margin:10px auto;display:inline-block;font-size;font-size:30px"><b>You have:  <span style="font-weight:600;color:green;font-size:50px"><?=$data['account_balance']?> $</span></b></div>
<div class="infobox t-left"><b><?=$lang['b_192']?>:</b> <?=($data['premium'] > 0 ? $lang['b_194'] : $lang['b_193'])?><?if($data['premium'] > 0){?><br /><b><?=$lang['b_159']?>:</b> <?=date('d-m-Y H:m', $data['premium'])?><?}?></div>
	<div class="vip_t_row">
        ...
        <ul>
			...
			<li class="desc_colum">Coupons</li>
